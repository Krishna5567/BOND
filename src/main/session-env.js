...
// Bond — and claude decides which it is purely from the environment.
//
// Launch Bond from a shell that is itself inside a claude session (`open -a
// Bond` from a claude tile, `npm start` in one) and that conversation's
// variables land in Bond's environment, and from there in every session Bond
...
// This is an explicit inventory, not a detector for arbitrary secrets. Saved
...
// Only diagnostics generated by Bond use this. A child's terminal output is

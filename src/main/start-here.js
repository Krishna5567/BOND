...
This is **${name}**, the folder you just gave Bond.

Bond can read and change things in here, and nowhere else on ${machine}. That is
the whole safety model, and it is why every session asks you for a folder first.

## Bond runs agents. It isn't one.

Bond is the desk. The thinking is done by an agent: Claude Code, Codex and a
few others. Each one signs in with **your own account**, so there is no Bond
...
You can delete this file whenever you like. It is yours now, and Bond will not
put it back.
`;
}

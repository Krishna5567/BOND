...
const THEMES = [['operator', 'Operator']];
...
function buildMenuTemplate({
  open, send, newWindow, platform = process.platform, name = 'Bond', theme = 'operator', recents = [],
} = {}) {
  const mac = platform === 'darwin';
  const cmd = (label, command, extra = {}) => ({ label, ...extra, click: () => send(command) });
  const link = (label, url) => ({ label, click: () => open(url) });

  // Two of these are the whole reason the branch exists. About Bond carries no
  // role, because `role: 'about'` is the stock panel; it opens Bond's own About
  ...
  const helpSubmenu = [
    ...
    SEP,
    link('Bond on GitHub', LINKS.repo),
    link('★ Star Bond', LINKS.repo),
...
function installAppMenu({ Menu, shell, app: electronApp, send, newWindow, theme, recents }) {
  const template = buildMenuTemplate({
    ...
    send,
    newWindow,
    name: (electronApp && electronApp.name) || 'Bond',
    theme,
    recents,
  });

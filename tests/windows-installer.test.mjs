...
test('the installer offers Bond under "Open with" for exactly the extensions open-with routes', () => {
  const added = [...code.matchAll(/!insertmacro BondOpenWith "([^"]+)"/g)].map((m) => m[1]);
  const removed = [...code.matchAll(/!insertmacro BondOpenWithRemove "([^"]+)"/g)].map((m) => m[1]);
...
test('installing Bond never takes a file type\'s default', () => {

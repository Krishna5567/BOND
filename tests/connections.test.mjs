...
// connections.js builds every file name with path.join, so they come out native
// (C:\proj\.mcp.json on Windows). Roots, expected names and the fake disk's
// keys are all built the same way, so a key written here is the key it looks up.
const path = require('path');
const HOME = path.resolve('/home/u');
const PROJ = path.resolve('/proj');
const proj = (rel) => path.join(PROJ, rel);
const home = (rel) => path.join(HOME, rel);
...
test('deliveryPlan user scope: claude goes via its own CLI, codex block lands in ~/.codex', () => {
  ...
  assert.equal(codex.file, home('.codex/config.toml'));
  const cursor = plan.find((s) => s.agent === 'cursor');
  assert.equal(cursor.file, home('.cursor/mcp.json'));
});
...
test('deliveryPlan user scope: grok’s notebook is ~/.grok/config.toml', () => {
  ...
  assert.equal(plan[0].file, home('.grok/config.toml'));
});
...
test('readNotebooks + coverage: who has it, who is missing it', () => {
  const io = memIo({
    ...
    [home('.cursor/mcp.json')]: JSON.stringify({ mcpServers: { notion: NOTION, n8n: { command: 'x' } } }),
    [home('.codex/config.toml')]: '[mcp_servers.notion]\ncommand = "npx"\n',
    [home('.grok/config.toml')]: '[cli]\ninstaller = "internal"\n\n[mcp_servers.notion]\ncommand = "npx"\n',

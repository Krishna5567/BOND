...
// what PowerShell is told is pinned in windows-shell.test.mjs.
const NO_ZSH = process.platform === 'win32' && 'runs the suffix in a real /bin/zsh, which Windows does not have';
...
test('a real zsh emits the sequence, with the real exit code', { skip: NO_ZSH }, () => {
...
test('a command that ends the shell reports nothing here, and that is fine', { skip: NO_ZSH }, () => {
  let out = '';
  try {
...
test('a real zsh reports the failing stage of a pipeline', { skip: NO_ZSH }, () => {
...
test('nothing of the sequence is left visible in the output', { skip: NO_ZSH }, () => {
...
test('curl | bash reports zero even when curl failed', { skip: NO_ZSH }, () => {
...
test('spawned for real: the code is reported and the output survives', { skip: NO_ZSH }, () => {
...
test('a failing spawned command still reports', { skip: NO_ZSH }, () => {

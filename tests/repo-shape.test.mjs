...
// Everything Bond is, everything that builds or explains it. Nothing else.
...
test('nothing is published from outside the folders that make Bond', () => {
  const stray = tracked.filter((f) => f.includes('/')
    ? !ALLOWED_DIRS.has(f.split('/')[0])
    : !ALLOWED_ROOT.has(f));
  assert.deepEqual(stray, [], stray.length
    ? `These are tracked but belong to no part of Bond:\n  ${stray.join('\n  ')}\n\n`
...
test('source and app distributions carry the same complete Apache license', () => {
  ...
  assert.match(license, /Copyright 2026 Bond AI/);
